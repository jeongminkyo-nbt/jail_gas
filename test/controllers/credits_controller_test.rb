...
class CreditsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create credit" do
    assert_difference('Credit.count') do
      post credits_url, params: { credit: { cost: @credit.cost, date: @credit.date, name: @credit.name, status: @credit.status } }
    end
  ...
  test "should update credit" do
    patch credit_url(@credit), params: { credit: { cost: @credit.cost, date: @credit.date, name: @credit.name, status: @credit.status } }
